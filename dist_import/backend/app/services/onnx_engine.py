...
class ONNXEngine:
    ...
    def _postprocess_onnx(self, outputs: List[np.ndarray], meta: Dict[str, Any]) -> List[Dict[str, Any]]:
        # Attempt to parse common YOLOv8-seg ONNX export: (1, N, 4+nc[+1]) and optionally mask proto
        ...
        # Heuristic: columns = [x,y,w,h,(obj?), class...]
        x, y, w, h = preds[:, 0], preds[:, 1], preds[:, 2], preds[:, 3]
        rest = preds[:, 4:]
        if rest.shape[1] == num_tools + 1:
            obj = rest[:, 0:1]
            cls_scores = rest[:, 1:]
            scores = (obj * cls_scores)
        elif rest.shape[1] == num_tools:
            scores = rest
        else:
            # fallback: take last num_tools columns
            scores = rest[:, -num_tools:]
        cls_ids = np.argmax(scores, axis=1)
        cls_conf = scores[np.arange(scores.shape[0]), cls_ids]
